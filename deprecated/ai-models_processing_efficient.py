...
# Consistent settings
path2obs = 'sensorData/timeseries_network2'   # Synoptic API JSON
...
def data_loading(path2obs, case_name, path2model):
    ...
    for number, site in enumerate(pd.Series(obs.keys())):
        ...
        # get uncorrected observation dataframe
        uncorr = obs[site]
        # copy and clean column names
        df = uncorr.copy()
        df.columns = df.columns.str.replace('_set_1d', '', regex=False).str.replace('_set_1', '', regex=False)
        ...
        for count, step in enumerate(model.step.data):
            ...
            ### CHANGE create lookup dictionary once
            # calculating model variables based on formulas in var_ref (from csv)
            var_formula_map = dict(zip(var_ref["variable"], var_ref["formula"]))
            for variable in var_ref["variable"]:
                if variable in varlist:
                    exec("tempdf[variable] =" + var_formula_map[variable])
        ...
        ### CHANGE, concatenate once after list is built 
        # concatenate all timesteps at once
        data[(df.attrs["STID"], (lon, lat))]["model"] = pd.concat(tempdf_list, ignore_index=False)
